...
flowchart TD
  ...
  T1A --> T1B[T1 readiness: loop try_lock(7); if success unlock(7), sleep; exit when try_lock(7) fails]

  %% T2 Startup
  P0 --> T2A[T2: lock(2), lock(4), lock(7)]
  T2A --> T2B[T2 readiness: loop try_lock(6); if success unlock(6), sleep; exit when try_lock(6) fails]

  %% Begin transactions
  T1B --> T1C[T1: BEGIN; READ COMMITTED]
  T2B --> T2C[T2: BEGIN; REPEATABLE READ (snapshot fixed)]
